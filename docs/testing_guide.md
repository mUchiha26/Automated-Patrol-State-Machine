...
1. Nodes in `automated_patrol_state_machine/`.
...
```bash
source /home/yass/turtlebot4_ws/src/automated_patrol_state_machine/.venv/bin/activate
```

### 5. Quality Gates (Stage A)

Execute in package root (`~/turtlebot4_ws/src/automated_patrol_state_machine`):
...
```bash
ros2 launch automated_patrol_state_machine cyclic_patrol.launch.py
```

Verify:

1. Node starts as `/patrol_executor_node`.
...
```bash
ros2 launch automated_patrol_state_machine environment_monitor.launch.py
```
...
```bash
ros2 launch automated_patrol_state_machine evidence_capture.launch.py
```
...
```bash
ros2 launch automated_patrol_state_machine full_system.launch.py
```

Verify nodes:

```bash
ros2 node list | grep -E "patrol_executor_node|environment_monitor_node|evidence_capture_node|alert_dispatcher_node|cli_feedback_node|mission_controller_node"
```
...
1. `patrol_executor_node` waits for Nav2 action server before sending goals and reports TF readiness issues as warnings.
